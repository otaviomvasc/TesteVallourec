...
if __name__ == '__main__':
    ...
    #Var Linearidade
    var_lin = p.LpVariable.dicts("linearidade", atividades, cat=p.LpContinuous)
    ...
    for i in fim_atividade:
        for j in atividades:
            ...
            prob += (var_lin[j] <= Upper * var_seq[i][j])
            prob += (var_lin[j] >= Lower * var_seq[i][j])
            prob += (var_lin[j] <= fim_atividade[j] - Lower * (1 - var_seq[i][j]))
            prob += (var_lin[j] >= fim_atividade[j] - Upper * (1 - var_seq[i][j]))
        ...
        prob += (fim_atividade[j] >= var_lin[j] + tempo_atv[j])
